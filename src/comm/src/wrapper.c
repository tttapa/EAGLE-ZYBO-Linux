...
#define OFFSET_NAV                                                             \
    (0xFFFF6000)  //register to check if we are in navigation mode or RC mode
...
int attitude(int arg) {
    ...
    switch (arg) {
        case 0: return rmem(R_OFFSET_THRUST); break;
        case 1: return rmem(R_OFFSET_X); break;
        case 2: return rmem(R_OFFSET_Y); break;
        case 3: return rmem(R_OFFSET_Z); break;
        default: printf("wrong attitude value\r\n"); return 0;
    }
}
int flight_mode() {
    //printf("mode = %d\r\n", rmem(OFFSET_NAV));
