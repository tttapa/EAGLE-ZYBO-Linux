...
class Quaternion {
  ...
  public:
    ...
    USE_CONSTEXPR ColVector<4> &asColVector() { return q; }
    USE_CONSTEXPR const ColVector<4> &asColVector() const { return q; }

    USE_CONSTEXPR Quaternion normalize() const { return {q / norm(q)}; }

    USE_CONSTEXPR Quaternion conjugate() const {
        return {{q[0], -q[1], -q[2], -q[3]}};
    }

    USE_CONSTEXPR Quaternion operator-() const { return conjugate(); }

    USE_CONSTEXPR static Quaternion quatmultiply(const Quaternion &q,
                                                 const Quaternion &r) {
        return {
    ...
    USE_CONSTEXPR Quaternion operator+(const Quaternion &r) const {
        return quatmultiply(*this, r);
    }

    USE_CONSTEXPR static Quaternion quatdifference(const Quaternion &q,
                                                   const Quaternion &r) {
        return q + (-r);
    }
    ...
    template <size_t C>
    USE_CONSTEXPR Matrix<3, C> rotate(const Matrix<3, C> &v) {
        Matrix<3, 3> M = {{
            {
                1 - 2 * sq(q[2]) - 2 * sq(q[3]),
                2 * (q[1] * q[2] + q[0] * q[3]),
    ...
    USE_CONSTEXPR real_t &operator[](size_t index) { return q[index]; }
    USE_CONSTEXPR const real_t &operator[](size_t index) const {
        return q[index];
    }

    USE_CONSTEXPR bool operator==(const Quaternion &other) const {
    ...
    USE_CONSTEXPR operator ColVector<4> &() { return q; }
    USE_CONSTEXPR operator const ColVector<4> &() const { return q; }

    USE_CONSTEXPR static Quaternion unit() { return {1, 0, 0, 0}; }
     ...
     */
    USE_CONSTEXPR static Quaternion fromDirection(ColVector<3> vec) {
        /*
